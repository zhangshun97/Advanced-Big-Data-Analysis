# H1: predict id_registration_city based on id_registration_district
...
# create features
indexer = StringIndexer(inputCol="_c10", outputCol="c22")
indexer = indexer.fit(total)
train = indexer.transform(train)
val = indexer.transform(val)
test = indexer.transform(test)
# create label
indexer = StringIndexer(inputCol="_c9", outputCol="label")
indexer = indexer.fit(total)
train = indexer.transform(train)
val = indexer.transform(val)
test = indexer.transform(test)
...
with open('H1_15300180012_output.txt', 'a') as f:
    f.write('\n \n')
    f.write('jq_H1_15300180012b_output_naive_bayes\n')
