# H3: predict the last ID number based on gender, birth date and id registration district
...
#   format="csv", sep="\t", inferSchema="true", header="false")
# train = train.rdd.map(lambda x: [x[10], x[8], x[6], x[1]%10]).toDF(["_c10", "_c8", "_c6", "_c1"])
# val = val.rdd.map(lambda x: [x[10], x[8], x[6], x[1]%10]).toDF(["_c10", "_c8", "_c6", "_c1"])
...
val = spark.read.load("../../../../val/part-00002",
  format="csv", sep="\t", inferSchema="true", header="false")
total = train.union(val)
total = total.rdd.map(lambda x: [x[10], x[8], x[6], x[1]%10]).toDF(["_c10", "_c8", "_c6", "_c1"])
...
with open('H3_15300180012_output_df.txt', 'a') as f:
    f.write('\n \n')
    f.write('jq_H31_15300180012_output_naive_bayes\n')
